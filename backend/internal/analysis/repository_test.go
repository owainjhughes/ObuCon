...
// Expected query strings are regexes matched against the SQL GORM generates, which
// quotes a select list only when it is a single column: `SELECT "lemma"` but
...
func TestUpsertKnownWordFillsGradeLevelFromDictionary(t *testing.T) {
	...
	// GORM wraps Create in its default transaction.
	mock.ExpectBegin()
...
func TestUpdateKnownWordReportsMissingRows(t *testing.T) {
	tests := []struct {
		name         string
		rowsAffected int64
		wantErr      error
	}{
		{name: "row updated", rowsAffected: 1},
		{name: "no matching row", rowsAffected: 0, wantErr: gorm.ErrRecordNotFound},
	}
...
func TestRemoveKnownWordReportsMissingRows(t *testing.T) {
	tests := []struct {
		name         string
		rowsAffected int64
		wantErr      error
	}{
		{name: "row deleted", rowsAffected: 1},
		{name: "no matching row", rowsAffected: 0, wantErr: gorm.ErrRecordNotFound},
	}
